				...
				if (codeHashExample.IsSupported)
				{
					...
					if (!codeHashExample.IsBusy)
					{
						if (codeHashExample.LastResult != null)
						{
							...
							{
								GUILayout.Label("Generated Summary Hash: " + codeHashExample.LastResult.SummaryHash);
								if (codeHashExample.IsGenuineValueSetInInspector)
								{
									GUILayout.Label("Hash matches value from inspector: " + codeHashExample.SummaryHashMatches());
								}
								else
								{
